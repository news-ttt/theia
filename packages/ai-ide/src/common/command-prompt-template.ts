...
This reply is to tell the user to execute the \`theia-ai-prompt-template:show-prompts-command\` command that is available in the PlcIDE command registry.
...
This reply is for custom commands that are not registered in the PlcIDE command registry.
...
If a user asks for a PlcIDE command, or the context implies it is about a command in PlcIDE, return a response with \`"type": "theia-command"\`.
You need to exchange the "commandId".
The available command ids in PlcIDE are in the list below. The list of commands is formatted like this:
...
Here are the known PlcIDE commands:
...
If the user asks for a command that is not a PlcIDE command, return a response with \`"type": "custom-handler"\`.
...
There is no specific command available to "open the windows" in the provided PlcIDE command list.
